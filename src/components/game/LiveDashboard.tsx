...
const LiveDashboard: React.FC = () => {
  ...
  const MiniBadge: React.FC<{
    ...
    <motion.button
      className="fixed bottom-20 right-4 md:bottom-4 md:right-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 z-40"
    ...
    >
      <div className="flex items-center space-x-2">
        <div className="relative">
          <Activity size={20} />
          <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full animate-pulse border-2 border-purple-500"></div>
        </div>
  ...
  return (
    <>
      {/* Mini Badge */}
      {!isOpen && <MiniBadge onClick={() => setIsOpen(true)} />}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className={`bg-gray-900 border border-gray-700 rounded-xl shadow-2xl transition-all duration-300 flex flex-col ${
                isExpanded ? 'w-full h-full max-w-7xl max-h-[95vh]' : 'max-w-4xl w-full'
              }`}
            ...
            >
              ...
              {/* Content */}
              <div className={`overflow-y-auto ${isExpanded ? 'p-8' : 'p-6'}`}>
                {/* Primary Stats Grid */}
